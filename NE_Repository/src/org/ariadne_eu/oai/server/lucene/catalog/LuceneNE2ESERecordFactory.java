...
public class LuceneNE2ESERecordFactory extends AbstractRecordFactory {
	private String dateField;
	private String identifierField;

	public LuceneNE2ESERecordFactory(Properties properties)
			throws IllegalArgumentException {
		super(properties);
		;
		dateField = properties
				.getProperty(RepositoryConstants.getInstance().OAICAT_SERVER_CATALOG_DATEFIELD);
		if (dateField == null) {
			throw new IllegalArgumentException(
					RepositoryConstants.getInstance().OAICAT_SERVER_CATALOG_DATEFIELD
							+ " is missing from the properties file");
		}
		identifierField = properties.getProperty(RepositoryConstants
				.getInstance().OAICAT_SERVER_CATALOG_IDFIELD);
		if (identifierField == null) {
			throw new IllegalArgumentException(
					RepositoryConstants.getInstance().OAICAT_SERVER_CATALOG_IDFIELD
							+ " is missing from the properties file");
		}
	}
